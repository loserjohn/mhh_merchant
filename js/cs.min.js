! function(u, D) {
	...
	var u = {
		...
		F = document.all && !document.addEventListener,
		o = n("//cdn.ronghub.com/css-sprite_bg-2.1.10.png"),
		t = n("//f2e.cn.ronghub.com/sdk/emoji-48.png"),
		i = {
			url: F ? o : t,
			size: 24,
			lang: "zh",
	...
	var r = {
			...
			hasKey: function(u, D) {
				var f = !1;
				if("object" === d.getType(u))
					for(var e in u) e === D && (f = !0);
				return f
			},
		...
		C = "具体信息请参考文档以及Demo示例: https://rongcloud.github.io/websdk-demo/emoji.html",
		c = {
			LogFactory: {
				0: {
					code: 0,
			...
			isShowError: !0,
			showErrorInfo: function(u) {
				console.error(u)
			},
			logger: function(u) {
				var D = u.code,
					f = this.LogFactory[D] || u,
					e = JSON.stringify(f),
		...
		d = {
			...
			checkConfigParam: function(u, D) {
				var f = this.checkLanguage(u.lang || i.lang, D),
					e = this.checkOptType(u.size || i.size, "number", 4, D),
		...
		g = function(u, D) {
			var f = (D || i.size) / (document.all && !document.addEventListener ? 24 : 25),
				e = (u = u.split(" "))[0],
				n = u[1];
	...
	var f = {
			...
			browserRedirect: function(u) {
				var D = navigator.userAgent.toLowerCase(),
					f = "ipad" == D.match(/ipad/i),
					e = "iphone os" == D.match(/iphone os/i),
					n = "midp" == D.match(/midp/i),
					F = "rv:1.2.3.4" == D.match(/rv:1.2.3.4/i),
					o = "ucweb" == D.match(/ucweb/i),
					t = "android" == D.match(/android/i),
					i = "windows ce" == D.match(/windows ce/i),
			...
			cloneObj: function(u) {
				...
				if("object" == typeof u) {
					if(window.JSON) D = JSON.stringify(u), f = JSON.parse(D);
					else
						for(var e in u) f[e] = "object" == typeof u[e] ? cloneObj(u[e]) : u[e];
					return f
				}
			}
		},
...
function(u) {
	...
	function e(u) {
		...
		for(var o = 0; o < F; o++) {
			var t = e.charCodeAt(o),
				i = null;
			if(t < 128) f++;
			else if(t > 127 && t < 2048) i = String.fromCharCode(t >> 6 | 192, 63 & t | 128);
			else if(63488 & t ^ !0) i = String.fromCharCode(t >> 12 | 224, t >> 6 & 63 | 128, 63 & t | 128);
			else {
	...
	function n(u) {
		var D, f, n, F, o, t = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",
			i = 0,
			a = 0,
			s = "",
			r = [];
	...
	function t(u, f, e) {
		var n = Math.ceil(d / f.chunk_size),
			F = Math.ceil(u.size / f.chunk_size),
			a = Math.min(f.chunk_size, u.size - d),
			s = u.slice(d, d + a),
			r = {
				chunk: n,
				chunks: F,
		...
		D(r, function(u, D) {
			...
			onCompleted: function(n) {
				d += a, l[u.uniqueName] = l[u.uniqueName] || [], l[u.uniqueName].push(n.ctx), d < u.size ? n.ctx ? t(u, f, e) : (d = 0, delete l[u.uniqueName]) : (d = 0, delete f.isChunk, delete f.headers["Content-Type"], D(r, function(u, D) {
					...
					ctx: l[u.uniqueName].join(","),
					name: u.name,
					size: u.size,
					filename: u.uniqueName
				}, f, e))
			},
			onError: function() {
				throw new Error("qiniu uploadChunk error")
	...
	var d = 0,
		...
		g = {
			form: function(u, D) {
				var f = new FormData;
				if(D.unique_key) {
					var e = u.name.substr(u.name.lastIndexOf(".")),
						n = a() + e;
		...
		h = function(u, D, f) {
			...
			F.onload = function() {
				...
				try {
					var t = e.toDataURL(u.type, D.quality),
	...
	var t, i = null,
		...
		g = function() {
			if(o.targetType == RongIMLib.ConversationType.CUSTOMER_SERVICE) {
				var u = new RongIMLib.TypingStatusMessage({
		...
		h = function(u) {
			var D = {
				showConversitionList: u.showConversitionList
			};
		...
		B = function(u, D, f, e) {
			var n = RongIMLib.ConversationType.CUSTOMER_SERVICE,
				F = u;
		...
		M = function(u) {
			for(var D = JSON.parse(JSON.stringify(u)), f = [], n = 0; n < D.length; n++) {
				var F = {
					sentTime: e.getTime(D[n].sentTime),
		...
		z = {
			file: function(u) {
				...
				return new RongIMLib.FileMessage({
					...
					size: u.size,
					type: e,
					fileUrl: u.downloadUrl
				})
			},
			image: function(u) {
				return new RongIMLib.ImageMessage({
					content: u.thumbnail,
					imageUri: u.downloadUrl
				})
			}
		},
		L = function(u) {
		...
		V = function(u, D) {
			var f = {
				...
				n = D.target || D.srcElement ? D.target || D.srcElement : D,
				F = n.value,
				t = "",
				i = o.formList;
		...
		q = function(u) {
			var D = {
				...
				f = u.target || u.srcElement ? u.target || u.srcElement : u,
				n = f.value,
				F = f.getAttribute("name"),
				o = f.getAttribute("data-label"),
				t = f.getAttribute("data-required"),
				i = f.getAttribute("data-pattern"),
				a = "";
			...
			if(0 != window.navigator.onLine) {
				...
				if(1 != c) {
					...
					RongIMClient.setConnectionStatusListener({
						...
						onSuccess: function(u) {
							f.getCurrentUser && f.getCurrentUser({
								userId: u
							}), console.log("链接成功，用户id：" + u), 1 == n && f.enterConversation && f.enterConversation(), n++
						},
	...
	u.init = function(D) {
		u.config = D, K(D, {
			getInstance: function(D) {
				d(function() {
					if(u.config.templates)
						for(var D in u.config.templates) {
		...
		u.stopPropagation();
		var D = r(".rongcloud-expressionWrap")[0],
			f = u.target || u.srcElement,
			n = r(".rongcloud-text")[0];
		...
		if(-1 != f.className.indexOf("rongcloud-animate")) f.className = f.className.replace(" rongcloud-animate", ""), clearTimeout(i);
		else {
		...
		var D = u.currentTarget.querySelector("video"),
			f = u.currentTarget.querySelector(".play-btn");
		...
		if(n) {
			var t = e.getFormValue(D),
				i = new RongIMClient.RegisterMessage.LeaveMessage(t);
			p(i, function() {
				w(), j()
			})
		}
		...
		var D = {
			imageUrl: (u.target || u.srcElement).getAttribute("data-img")
		};
		...
		var u = new RongIMClient.RegisterMessage.DownloadHistoryMessage({
			begin: 0
		});
		p(u, function() {})
		...
		var D = u.target || u.srcElement; - 1 != D.className.indexOf("rongcloud-group-tag-active") ? D.className = "rongcloud-group-tag rongcloud-tag-item" : D.className = "rongcloud-group-tag rongcloud-tag-item rongcloud-group-tag-active"
	}
